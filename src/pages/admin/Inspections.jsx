    ...
    <div style={{ padding: "28px", maxWidth: "1400px", margin: "0 auto", fontFamily: "Segoe UI, sans-serif" }}>
      ...
      <div style={{ background: "linear-gradient(135deg, #0c3b6b 0%, #0284c7 100%)", borderRadius: "14px", padding: "22px 28px", color: "#ffffff", marginBottom: "24px", boxShadow: "0 6px 20px rgba(12, 59, 107, 0.2)" }}>
        <h1 style={{ fontSize: "22px", fontWeight: 800, margin: "0 0 4px 0" }}>
          Field Inspections & Store Audits
        </h1>
      ...
      <div style={{ background: "#ffffff", borderRadius: "14px", border: "1px solid #e2e8f0", boxShadow: "0 4px 14px rgba(0,0,0,0.03)", overflow: "hidden" }}>
        ...
        <div style={{ overflowX: "auto" }}>
          {loading ? (
            <div style={{ padding: "40px", textAlign: "center", color: "#64748b" }}>Loading live inspection logs...</div>
          ) : inspectionLogs.length === 0 ? (
            <div style={{ padding: "40px", textAlign: "center", color: "#64748b" }}>No inspections recorded yet.</div>
          ) : (
            <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left" }}>
              <thead>
                <tr style={{ background: "#f8fafc", borderBottom: "1px solid #e2e8f0", color: "#475569", fontSize: "11.5px", textTransform: "uppercase", fontWeight: 700 }}>
                  <th style={{ padding: "14px 20px" }}>Inspection Code & Entity</th>
                  <th style={{ padding: "14px 20px" }}>Assigned Officer</th>
                  <th style={{ padding: "14px 20px" }}>Commodity</th>
                ...
                {inspectionLogs.map((item, idx) => (
                  <tr key={idx} style={{ borderBottom: "1px solid #f1f5f9", fontSize: "12.5px", color: "#1e293b" }}>
                    <td style={{ padding: "14px 20px", fontWeight: 700 }}>
                      {item.product?.manufacturer_name || "Commercial Lot"}
                      <div style={{ fontSize: "11px", color: "#0c3b6b", fontWeight: 600 }}>{item.inspection_code || `INSP-#${item.id}`}</div>
                    </td>
                    <td style={{ padding: "14px 20px", color: "#0284c7", fontWeight: 600 }}>
                      {item.checker_id ? `Inspector ID #${item.checker_id}` : "Central Metrology Wing"}
                    </td>
                    <td style={{ padding: "14px 20px", color: "#475569" }}>
                      {item.product?.name || `Product #${item.product_id}`}
                    </td>
                    <td style={{ padding: "14px 20px" }}>
                      {item.scheduled_at ? new Date(item.scheduled_at).toLocaleDateString("en-IN") : "N/A"}
                    </td>
                    <td style={{ padding: "14px 20px" }}>
                      <span style={{
                        fontSize: "11px",
                        ...
                        padding: "4px 10px",
                        borderRadius: "6px",
                        background: item.status === "COMPLETED" || item.status === "COMPLIANT" ? "#f0fdf4" : item.status === "VIOLATION" || item.status === "NON_COMPLIANT" ? "#fef2f2" : "#fef3c7",
                        color: item.status === "COMPLETED" || item.status === "COMPLIANT" ? "#166534" : item.status === "VIOLATION" || item.status === "NON_COMPLIANT" ? "#b91c1c" : "#92400e"
                      }}>
                        {item.status}
                      </span>
                    </td>
                    <td style={{ padding: "14px 20px", textAlign: "right" }}>
                      <button 
                        onClick={() => alert(`Inspection Details: ${item.inspection_code}\nStatus: ${item.status}\nRemarks: ${item.remarks || 'None'}`)}
